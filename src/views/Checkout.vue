      ...
      <div class="checkout-grid">
        ...
        <div class="forms">
          <section class="form-section">
            ...
            <div class="form-row">
              <input v-model="form.zip" placeholder="PLZ *" required :disabled="loading">
...
const canSubmit = computed(() => {
  const required = [form.firstName, form.lastName, form.email, form.street, form.zip, form.city]
  return required.every(f => f.trim()) && acceptTerms.value && cartItems.value.length > 0
})

const submitOrder = async () => {
  const errors = []
  if (!form.firstName.trim()) errors.push('Vorname erforderlich')
  if (!form.lastName.trim()) errors.push('Nachname erforderlich')
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) errors.push('Gültige E-Mail erforderlich')
  if (!form.street.trim()) errors.push('Straße erforderlich')
  if (!/^[0-9]{5}$/.test(form.zip)) errors.push('5-stellige PLZ erforderlich')
  if (!form.city.trim()) errors.push('Stadt erforderlich')
  if (!acceptTerms.value) errors.push('AGB akzeptieren')
  if (cartItems.value.length === 0) errors.push('Warenkorb leer')
  ...
  try {
    ...
    const orderData = {
      ...
      payment: { method: paymentMethod.value, status: 'pending' },
      order: {
    ...
    if (!response.ok) throw new Error(`Bestellung fehlgeschlagen (${response.status})`)
    ...
    cartStore.clearCart()
    router.push({ name: 'OrderConfirmation', params: { orderNumber: result.orderNumber } })
